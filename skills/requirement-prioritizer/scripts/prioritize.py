...
"""
需求优先级评估独立脚本
调用 requirement-prioritizer skill 对需求进行评分
"""
...
def call_claude_skill(requirements):
    ...
    构造提示词，让Claude使用requirement-prioritizer评估优先级
    """
    # 将需求数据转为紧凑的JSON
    prompt_data = json.dumps(requirements, ensure_ascii=False, indent=2)

    prompt = f"""请使用 requirement-prioritizer skill 评估以下需求的优先级：

{prompt_data}

请直接输出评估结果的JSON，不要有其他内容。"""
...
def main():
    if len(sys.argv) < 3:
        print("用法: python3 prioritize.py <input.json> <output.json>")
        print("\n示例:")
        print("  python3 prioritize.py analyzed.json prioritized.json")
        sys.exit(1)
    ...
    print("调用 requirement-prioritizer skill...")
