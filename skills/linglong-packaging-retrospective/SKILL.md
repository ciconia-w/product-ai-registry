...
These artifacts are local run records for the target repository.
They must also be summarized into the shared Linglong reference maintenance surface so later agents can discover the pitfalls.
...
After saving the local retrospective:

1. keep the full JSON and Markdown under `.ai-registry/linglong-retrospectives/`
2. create a concise shared summary for later discovery
3. append or submit that summary to the shared Linglong reference maintenance path

The local files are the full per-run record.
The shared report is the searchable cross-run pitfall layer.
